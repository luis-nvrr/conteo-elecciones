package org.openjfx.Estructuras;
...
public class TSBHashtable<K,V> implements Map<K,V>, Cloneable, Serializable
{
    ...
    public V remove(Object key) 
    {
        ...
        int ik = this.h((K)key);
        int is = this.search_for_index((K)key,ik);

        V old = null;
        if(is != -1)
        {
            Entry<K, V> x = (Entry<K, V>)table[is];
            old = x.getValue();
            x.setState(TOMBSTONE);
            this.count--;
            this.modCount++;
        }
        return old;
     ...
     */ 
    @Override
    protected TSBHashtable<K, V> clone() throws CloneNotSupportedException
    {
        // HACER...
        TSBHashtable<K, V> t = (TSBHashtable<K, V>)super.clone();

        Object[] temp = new Object[this.table.length];
    ...
    public boolean contains(Object value)
    {
        ...
        {
            Entry<K, V> entry = (Entry<K, V>) table[i];
            if(((V)value).equals(entry.getValue())) return true;
        }
        return false;
    }
    ...
    private class KeySet extends AbstractSet<K> 
    {
        ...
        @Override
        public boolean remove(Object o) { return (TSBHashtable.this.remove(o) != null); }
        
        @Override
        public void clear() { TSBHashtable.this.clear(); }
        
        private class KeySetIterator implements Iterator<K>
        {
            // REVISAR y HACER... Agregar los atributos que necesiten...
            private int current_entry;
            private int next_available_entry;
            ...
            {
                // HACER...
                Object t[] = TSBHashtable.this.table;

                if(TSBHashtable.this.isEmpty()){ return false; }
                if( current_entry >= t.length) { return false;}

                next_available_entry = current_entry +1;
                while(next_available_entry < t.length){
                    if(((Entry)t[next_available_entry]).getState() == CLOSED){
                        return true;
                    }
            ...
            public K next() 
            {
                ...
                Entry next = (Entry) t[next_available_entry];

                current_entry = next_available_entry;
            ...
            public void remove() 
            {
                ...
                Entry<K, V> oldEntry = (Entry<K, V>) TSBHashtable.this.table[current_entry];
                K oldKey = oldEntry.key;
                TSBHashtable.this.remove(oldKey);
                current_entry--;
                ...
                //TSBHashtable.this.count--; MODIFICADO

                // fail_fast iterator...
                //TSBHashtable.this.modCount++; MODIFICADO
            }     
        }
    }
    ...
    private class EntrySet extends AbstractSet<Map.Entry<K, V>> 
    {
        ...
        public boolean contains(Object o) 
        {
            ...
            if(TSBHashtable.this.containsKey(entryKey)){
                V value = TSBHashtable.this.get(entryKey);
        ...
        public boolean remove(Object o) 
        {
            ...
            return TSBHashtable.this.remove(entryKey) != null;  // llama a remove de la tabla, con la clave del entrry
        }
